...
function everyId(cabinet: Cabinet): NodeId[] {
  const ids = cabinet.library.map((channel) => channel.id);
  eachNode(cabinet.library, (node) => ids.push(node.id));
  return ids;
}

describe("withFreshIds", () => {
  it("gives every channel, folder and note an id of its own", () => {
  ...
  it("uses the prefix that matches what it is renaming", () => {
    const cabinet: Cabinet = {
      library: [makeChannel("One", [makeFolder("Inner", [makeNote()])])],
      tags: [],
    };
  ...
  it("keeps the shape, the content and the tags exactly as they were", () => {
    const cabinet: Cabinet = {
      library: [makeChannel("Reading", [makeFolder("Essays", [makeNote({ title: "Kept" })])])],
