...
describe("summarizeCabinet", () => {
  it("counts shelves, folders, notes and tags across the whole tree", () => {
    const summary = summarizeCabinet(makeLibrary(), [makeTag("To read", TAG_PALETTE[0])]);

    expect(summary).toEqual({ shelves: 2, folders: 2, notes: 4, tags: 1 });
  });
  ...
  it("ignores a link that is still being read", () => {
    ...
    const library = [makeShelf("Inbox", [makeNote(), pending])];
