...
export function TransferModal({
  ...
  return (
    <FormModal size="md" heading={copy.sync.heading} onClose={onCancel}>
      <div className="cab-block cab-summary">
        <CabinetCounts summary={summarizeCabinet(library, tags)} />
      </div>

      <div className="cab-panes">
        <Field label={copy.sync.places}>
          <div className="places">
            {sync.destinations.length === 0 ? (
              <p className="places-empty">{copy.sync.placesEmpty}</p>
            ) : (
        ...
        <Field label={copy.transfer.heading}>
          {staged ? (
            ...
            <div className="cab-pair">
              <button type="button" className="tile" onClick={onExport}>
                <span className="tile-mark">
                  <Icon name="download" />
                </span>
                <span className="tile-name">{copy.transfer.exportLabel}</span>
                <span className="tile-sub">{copy.transfer.exportSub}</span>
              </button>

              <label
                className="tile cab-drop"
              ...
              >
                ...
                <span className="tile-name">{copy.transfer.importLabel}</span>
                <span className="tile-sub">{copy.transfer.importSub}</span>
                <input
                  type="file"
                  accept="application/json,.json"
                  className="cab-file-input"
                  aria-label={copy.transfer.drop}
                  onChange={(event) => {
